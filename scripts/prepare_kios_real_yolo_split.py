...
from uav_safety.real_landing_dataset import discover_samples
...
def main() -> None:
    ...
    for item in assignments:
        ...
        if item.split == "embargo":
            continue
        shutil.copy2(item.sample.image_path, args.out / "images" / item.split / item.sample.image_path.name)
        shutil.copy2(item.sample.label_path, args.out / "labels" / item.split / item.sample.label_path.name)
    ...
    summary = (
        ...
        "adjacent-frame leakage into the test segment.\n\n"
