...
FIT_SEED = 495495
CALIBRATION_SEED = 506506
TRANSFER_SEED = 517517
VALIDATION_SEED = 528528
FRAMES_PER_SEQUENCE = p9.FRAMES_PER_SEQUENCE

FIT_FAMILIES = tuple(range(236, 242))
CALIBRATION_FAMILIES = tuple(range(242, 272))
TRANSFER_FAMILIES = tuple(range(272, 288))
VALIDATION_FAMILIES = tuple(range(288, 304))
...
def _build_candidate(fit_raw, calibration_raw, git_sha: str):
    ...
    candidate = {
        "schema": "aegisland.phase11.p11.candidate-freeze.v1",
        "git_sha": git_sha,
...
def _manifest(out: Path, files: list[str], stage: str, git_sha: str) -> None:
    payload = {
        "schema": "aegisland.phase11.p11.manifest.v1",
        "stage": stage,
        "git_sha": git_sha,
        "simulation_only": True,
        "safety_acceptance": False,
...
def _load_candidate(path: Path):
    ...
    if candidate.get("schema") != "aegisland.phase11.p11.candidate-freeze.v1":
        raise SystemExit("invalid P11 candidate schema")
    if candidate.get("transfer_seed_unseen_at_freeze") != TRANSFER_SEED:
        raise SystemExit("P11 transfer seed mismatch")
    if candidate.get("validation_seed_unseen_at_freeze") != VALIDATION_SEED:
        raise SystemExit("P11 validation seed mismatch")
    return candidate
...
def _summarize(evaluated, reference, candidate, role: str, seed: int, require_transfer_minimums: bool):
    ...
    result["schema"] = "aegisland.phase11.p11.result.v1"
    return result
...
def transfer(out: Path, candidate_path: Path, git_sha: str):
    ...
    result = _summarize(transfer_df, reference, candidate, "phase11_p11_seen_transfer", TRANSFER_SEED, True)
...
def validate(out: Path, candidate_path: Path, git_sha: str):
    ...
    reference_raw = _raw("fit", FIT_SEED, FIT_FAMILIES, FIT_DOMAINS)
    reference = p9.add_p9_continuity(reference_raw, candidate["velocity_caps"], candidate["innovation_scales"])
    result = _summarize(validation, reference, candidate, "phase11_p11_frozen_candidate_validation", VALIDATION_SEED, False)
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Run Phase 11 P11 powered direct-conformal replication")
...
def main() -> None:
    ...
    if args.stage == "freeze":
        ...
        print("P11_CANDIDATE_FREEZE=" + json.dumps({"calibration_group_rows": candidate["calibration_group_rows"], "velocity_caps": candidate["velocity_caps"], "innovation_scales": candidate["innovation_scales"]}, sort_keys=True))
        return
    if args.candidate is None:
        raise SystemExit("--candidate required")
    if args.stage == "transfer":
        result = transfer(args.out, args.candidate, args.git_sha)
        print("P11_TRANSFER_GATES=" + json.dumps(result["gates"], sort_keys=True))
        return
    result = validate(args.out, args.candidate, args.git_sha)
    print("P11_VALIDATION_GATES=" + json.dumps(result["gates"], sort_keys=True))
