...
def test_phase6d_episode_is_deterministic():
    temporal = fit_synthetic_calibrator(seed=616161, samples_per_condition=24)
    component = fit_component_calibrator(seed=616161, samples_per_condition=32)

    a = run_phase6d_episode(123456, "occlusion", temporal, component)
    b = run_phase6d_episode(123456, "occlusion", temporal, component)

    assert a.to_dict() == b.to_dict()
    assert a.architecture == "image_aegis_phase6d"


def test_phase6d_consistency_metrics_are_bounded():
    temporal = fit_synthetic_calibrator(seed=626161, samples_per_condition=24)
    component = fit_component_calibrator(seed=626161, samples_per_condition=32)
    result = run_phase6d_episode(654321, "mixed", temporal, component)
    ...
    assert 0 <= result.hard_altitude_alias_frames <= result.frames
    assert result.mean_altitude_disagreement_sigma >= 0.0
