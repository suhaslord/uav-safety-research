(() => {
  ...
  function installBrandPolish() {
    if (!document.getElementById("aegisBrandPolish")) {
      ...
      style.textContent = `
        #refreshStatus { display: none !important; }
        .aegis-boot {
          position: fixed;
          inset: 0;
          z-index: 9999;
  ...
  function enhanceLineage() {
    ...
    const eras = [
      {number:"01",title:"Foundation",description:"Protocol through the frozen V1–V3 supervisor evolution.",items:items.slice(0,4)},
      {number:"02",title:"Perception + robustness",description:"Stress testing, image perception, and the frozen Phase 6B study.",items:items.slice(4,7)},
      {number:"03",title:"External validation",description:"Development stress, trace validation, and genuine PX4/Gazebo evidence.",items:items.slice(7,10)},
      {number:"04",title:"Current frontier",description:"Phase 9 camera evidence and the frozen Phase 10 follow-up stay inspectable in the archive.",items:items.slice(10,11),current:true}
    ];
  ...
  function buildMobileMenu() {
    ...
    sheet.innerHTML = `
      ...
      <div class="mobile-menu-secondary">
        <a href="/phases/">Research archive</a>
        <a href="/phases/phase10/">Phase 10</a>
  ...
  window.addEventListener("load", () => { syncHeaderSurface(); releaseLoader(); }, {once:true});
