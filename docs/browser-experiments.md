...
The interface adds plain-language questions for all phases, a site-wide introduction and glossary, sliders with numeric input, and three starting scenarios. Presets are synthetic examples, not validated flight conditions. Sample count and seed are under repeatability settings. The browser keeps up to four distinct completed runs per phase, plus the current result, in memory. It shows metric values in their original units and the settings that differ. Exact duplicate inputs are not added twice. Each phase has a separate stack; rows from different phase methods are never pooled. Runs can be removed individually or cleared per phase. This is a comparison of examples, not a statistical significance test. Reloading clears the stacks. The full comparison can be exported as JSON. A selectable measurement can be overlaid across runs; missing measurements are omitted and unequal-length series stop at their own endpoints.
...
Comparisons now span the full lab width. Runs are grouped by method family and exact horizontal/vertical axis labels, allowing related phases to share a stack. Only series labels present in every compared run are offered in the shared graph. Metrics remain phase-specific: unavailable values are not substituted with zero. Identical curves overlap. These are illustrative method comparisons, not cross-phase rankings.
