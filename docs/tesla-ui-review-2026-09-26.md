...
| Product pages use one large title, a quiet subline, and visual hierarchy. | Archive search text overlapped its placeholder, its first category label was clipped on phones, and the glossary interrupted the records; the archive hero was centered and dense; phase templates stacked small metadata labels and gray panels. | Fixed the search field and category chip, moved the glossary below the archive, shortened and aligned its opening, and standardized typography, media framing, and verdict panels across phase families. |
