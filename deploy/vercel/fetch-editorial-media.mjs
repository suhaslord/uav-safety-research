...
function validateJpeg(buffer, label = 'JPEG') {
  if (buffer.length <= minJpegBytes) throw new Error(`${label} is too small (${buffer.length} bytes)`);
  if (buffer[0] !== 0xff || buffer[1] !== 0xd8) throw new Error(`${label} is missing the JPEG SOI marker`);
  ...
  while (offset < buffer.length) {
    if (buffer[offset] !== 0xff) throw new Error(`${label} has invalid marker alignment at byte ${offset}`);
  ...
  for (const url of asset.urls) {
    for (let attempt = 1; attempt <= attemptsPerSource; attempt += 1) {
      try {
        ...
        // Re-read before rename so a short/partial filesystem write is never published.
        const written = await fs.readFile(temporary);
        validateJpeg(written, `${asset.file} staged file`);
        await fs.rename(temporary, target);

        console.log(`✓ ${asset.label}: ${asset.file} (${dimensions.width}x${dimensions.height}, ${Math.round(buffer.length / 1024)} KiB)`);
        return;
      } catch (error) {
        lastError = error;
        console.warn(`Media fetch failed for ${asset.file} from ${url} (attempt ${attempt}/${attemptsPerSource}): ${error.message}`);
