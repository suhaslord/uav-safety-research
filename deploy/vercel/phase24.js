(() => {
  ...
  function groupedChart(rows,metric,title,description){
    const w=820,h=300,left=56,right=18,top=18,bottom=64,plotH=h-top-bottom,plotW=w-left-right;
    ...
    const bars=rows.map((row,index)=>{
      ...
      const baseX=cx-barW-2,phaseX=cx+2,labelY=h-27;
      const label=row.label==='Mixed stress'?'<tspan x="'+cx+'" dy="0">Mixed</tspan><tspan x="'+cx+'" dy="14">stress</tspan>':esc(row.label);
      return '<rect class="bar-baseline" x="'+baseX+'" y="'+(top+plotH-baseH)+'" width="'+barW+'" height="'+baseH+'" rx="3"><title>'+esc(row.label)+' baseline: '+percent(row.baseline[metric])+'</title></rect><rect class="bar-phase23" x="'+phaseX+'" y="'+(top+plotH-phaseH)+'" width="'+barW+'" height="'+phaseH+'" rx="3"><title>'+esc(row.label)+' Phase 23: '+percent(row.phase23[metric])+'</title></rect><text x="'+cx+'" y="'+labelY+'" text-anchor="middle">'+label+'</text>';
    }).join("");
  ...
  function deltaChart(rows,metric,id,title,description){
    const w=820,h=300,left=64,right=18,top=24,bottom=64,plotH=h-top-bottom,plotW=w-left-right;
    ...
    const bars=rows.map((row,index)=>{
      ...
      const cx=x+barW/2,label=row.label==='Mixed stress'?'<tspan x="'+cx+'" dy="0">Mixed</tspan><tspan x="'+cx+'" dy="14">stress</tspan>':esc(row.label);
      return '<rect class="'+cls+'" x="'+x+'" y="'+y+'" width="'+barW+'" height="'+height+'" rx="3"><title>'+esc(row.label)+': '+(value>=0?'+':'')+value.toFixed(1)+' percentage points</title></rect><text class="value" x="'+cx+'" y="'+(value>=0?y-6:y+height+15)+'" text-anchor="middle">'+(value>=0?'+':'')+value.toFixed(1)+'</text><text x="'+cx+'" y="'+(h-27)+'" text-anchor="middle">'+label+'</text>';
    }).join("");
  ...
  render().catch((error)=>{
    ...
    ["macroMapDelta","macroRecallDelta","mapWins","tailMapDelta"].forEach((id)=>{const node=document.getElementById(id);if(node)node.textContent="Unavailable";});
    ...
    const tail=document.getElementById("tailReadout");if(tail)tail.textContent="The severe-stress readout is unavailable. Open the reproducible report linked below.";
  });
})();
