  ...
  function start() {
    ...
    function render(data) {
      ...
      resultsEl.querySelector('.lab-save').onclick=()=>{
        ...
        if(saved.length>=4){status.textContent='Four compatible runs are saved. Remove one before adding another.';return;}
        saved.push({id:++stackId,data:structuredClone(result)});renderComparison();comparison.scrollIntoView({block:'start'});status.textContent='Run added. Try another setting or a related phase to compare on the same graph.';
      };
    ...
    phaseSelect.onchange=()=>{phase=phaseSelect.value;result=null;comparison.innerHTML='<h4>Your saved runs stay here</h4><p>Run this phase to compare it with saved runs that use the same measurement family and axes.</p>';controls();resultsEl.innerHTML='<div class="lab-empty"><h3>'+esc(questions[phase])+'</h3><p>'+esc(lessons[catalog[phase][1]])+'</p></div>'; };
