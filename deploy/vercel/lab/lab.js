(() => {
  ...
  let worker, requestId=0, result=null, timer, busy=false, stacks={},stackId=0;
  ...
  function start() {
    ...
    function renderComparison() {
      ...
      const saved=stacks[result.phase]||[];
      if(!saved.length){target.innerHTML='<h4>Build a comparison</h4><p>Add up to four runs from this phase, then change a setting and run again. Your latest result appears alongside them. Other phases keep separate comparisons.</p>';return;}
      const identical=saved.find(r=>JSON.stringify(r.data.inputs)===JSON.stringify(result.inputs));
      const runs=[...saved.map((r,i)=>({...r,name:'Run '+(i+1)})),...(!identical?[{id:'current',name:'Current',data:result}]:[])];
      ...
      target.innerHTML=`<h4>Compare ${runs.length} ${runs.length===1?'run':'runs'} · ${label(result.phase)}</h4><p>Same phase and units. These are individual examples, not a statistical test. Saved in this tab until you reload.</p><p class="lab-stack-scroll-hint">Scroll the table sideways to see every run on a small screen.</p><div class="lab-table lab-stack-table" tabindex="0" role="region" aria-label="Run comparison"><table><thead><tr><th scope="col">Measure / setting</th>${runs.map(r=>`<th scope="col">${r.name}${r.id!=='current'?`<button type="button" data-remove="${r.id}" aria-label="Remove ${r.name}">Remove</button>`:''}</th>`).join('')}</tr></thead><tbody>${metrics.map(m=>`<tr><th scope="row">${esc(explainMetric(m)[0])}${m.unit?' ('+esc(m.unit)+')':''}</th>${runs.map(r=>`<td>${fmt(r.data.metrics.find(x=>x.label===m.label)?.value??null)}</td>`).join('')}</tr>`).join('')}${changed.map(k=>`<tr class="lab-setting-row"><th scope="row">${esc(k==='noise'&&catalog[result.phase][1]==='context'?'Observation noise':fields[k]?.[0]||k)}</th>${runs.map(r=>`<td>${esc(fmt(r.data.inputs[k]))}</td>`).join('')}</tr>`).join('')}</tbody></table></div><details class="lab-stack-plot"><summary>Overlay a measurement across runs</summary><label>Measurement<select class="lab-stack-series"></select></label><figure class="lab-stack-chart"></figure><p>Each line is one run. Missing series are omitted; different run lengths end at their last reading.</p></details><div class="lab-output-actions"><button type="button" class="lab-export-stack">Download comparison JSON</button><button type="button" class="lab-clear">Clear this phase’s comparison</button></div>`;
      target.querySelectorAll('[data-remove]').forEach(button=>button.onclick=()=>{stacks[result.phase]=saved.filter(r=>r.id!==Number(button.dataset.remove));renderComparison();});
      target.querySelector('.lab-clear').onclick=()=>{stacks[result.phase]=[];renderComparison();};
      ...
      const names=[...new Set(runs.flatMap(r=>r.data.series.map(line=>line.label)))];
      ...
      select.onchange=plot;target.querySelector('.lab-stack-plot').ontoggle=event=>{if(event.target.open)plot();};
    }
