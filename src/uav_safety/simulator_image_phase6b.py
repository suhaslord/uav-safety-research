...
from .phase6b_fusion import (
    ComponentFusionDiagnostics,
    Phase6BComponentFusionAdapter,
    Phase6BComponentGateConfig,
)
...
class Phase6BEpisodeResult:
    ...
    lateral_component_abstentions: int
    lateral_component_abstention_rate: float
    altitude_component_abstentions: int
    altitude_component_abstention_rate: float
    lateral_reference_takeovers: int
    altitude_reference_takeovers: int
...
def run_phase6b_episode(
    ...
    *,
    ...
    """Run the Phase 6B component-selective image/Aegis architecture.

    This function intentionally does not modify or wrap the historical
    ``run_image_episode`` result path. It reuses the same simulated environment,
    pixel renderer, temporal tracker, robust image velocity filter, independent
    reference estimator, controller, and frozen V3 safety supervisor. The new
    layer supplies separate image-derived lateral and altitude reliability to a
    Phase-6B-only component fusion adapter.
    """
    ...
    fusion = Phase6BComponentFusionAdapter(
        sup_cfg,
        phase6_fusion_cfg,
        component_gate_cfg,
    )
    ...
    for i in range(steps):
        ...
        # Historical Phase 6 temporal state path remains unchanged.
        image_obs, temporal_diag = image_pipeline.update(frame)
        image_obs, velocity_diag = velocity_filter.update(image_obs)

        # Phase 6B computes component reliability independently on the same pixels.
    ...
    result = Phase6BEpisodeResult(
        seed=seed,
        condition=condition,
        architecture="image_aegis_phase6b",
        outcome=outcome,
...
def _trace_row(
    ...
    temporal_diag,
    velocity_diag,
    component_diag: ComponentFusionDiagnostics,
    fused,
    ref_obs,
    risk,
    decision,
    ...
    return {
        ...
        "temporal_abstained": bool(temporal_diag.abstained),
        "temporal_calibrated_confidence": float(temporal_diag.calibrated_confidence),
        "p_x_good": float(component_diag.p_x_good),
        "p_z_good": float(component_diag.p_z_good),
        "lateral_component_abstained": bool(component_diag.lateral_abstained),
        "altitude_component_abstained": bool(component_diag.altitude_abstained),
        "lateral_reference_takeover": bool(component_diag.lateral_reference_takeover),
        "altitude_reference_takeover": bool(component_diag.altitude_reference_takeover),
        ...
        "reference_vx": float(ref_obs.vx),
        "reference_vz": float(ref_obs.vz),
