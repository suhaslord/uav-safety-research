...
from .phase6b_fusion import ComponentFusionDiagnostics, Phase6BComponentGateConfig
from .reference_estimator import ReferenceObservation
from .supervisor_v3 import FusionResult, SupervisorV3Config


class Phase6CComponentFusionAdapter:
    """Phase 6C: preserve vertical-rate control during altitude abstention.

    Phase 6B showed that a low-confidence altitude estimate could trigger hundreds
    of reference takeovers. Because the landing controller regulates vertical
    motion from ``vz`` rather than ``z``, blending the lower-rate reference ``vz``
    on every altitude abstention can change descent dynamics even though the
    calibrated failure is specifically an altitude/scale failure.

    Phase 6C keeps the exact Phase 6B thresholds, lateral fallback, uncertainty
    handling, and frozen V3 supervisor evidence. The only change is that an
    altitude component takeover may blend ``z`` toward the independent reference
    while leaving the established Phase 6 control ``vz`` untouched.
    """
    ...
    def update(
        ...
        *,
        ...
        if altitude_abstained and reference_usable:
            ...
            # Deliberately preserve the established Phase 6 vertical-rate state.
            # p_z_good is calibrated against altitude error, not velocity error.
            vz = control.vz
