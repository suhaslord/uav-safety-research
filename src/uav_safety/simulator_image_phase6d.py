...
@dataclass
class Phase6DEpisodeResult(Phase6BEpisodeResult):
    hard_altitude_alias_frames: int
    mean_altitude_disagreement_sigma: float


def run_phase6d_episode(
    seed: int,
    condition: str,
    ...
    *,
    ...
    """Run the simulation-only Phase 6D estimator-consistency revision."""
    ...
    fusion = Phase6DComponentFusionAdapter(
        sup_cfg,
        phase6_fusion_cfg,
        component_gate_cfg,
        consistency_cfg,
    ...
    hard_altitude_alias_frames = 0
    interventions = 0
    reference_updates = 0
    temporal_confidences: list[float] = []
    px_values: list[float] = []
    pz_values: list[float] = []
    altitude_disagreement_sigma_values: list[float] = []
    ...
    for i in range(steps):
        ...
        hard_altitude_alias_frames += int(component_diag.hard_altitude_alias)
        altitude_disagreement_sigma_values.append(float(component_diag.altitude_disagreement_sigma))
    ...
    result = Phase6DEpisodeResult(
        seed=seed,
        condition=condition,
        architecture="image_aegis_phase6d",
        outcome=outcome,
        ...
        hard_altitude_alias_frames=int(hard_altitude_alias_frames),
        mean_altitude_disagreement_sigma=float(
            np.mean(altitude_disagreement_sigma_values) if altitude_disagreement_sigma_values else 0.0
        ),
    )
    return (result, trace) if return_trace else result


def _trace_row_d(*args) -> dict:
    component_diag = args[6]
    row = _trace_row(*args)
    row["hard_altitude_alias"] = bool(component_diag.hard_altitude_alias)
    row["altitude_disagreement_sigma"] = float(component_diag.altitude_disagreement_sigma)
    return row
